...
export default DropboxDataStoreAdapter("3pjra0nxwgejhqm", App);
...
function DropboxDataStoreAdapter(key, App) {
  ...
  function getTable(type) {
    return new Ember.RSVP.Promise(function(resolve, reject) {
      datastore.then(function(datastore) {
        // TMF: Patch for AppKit requirement
        var camelized = Ember.String.camelize(type.typeKey);        
        var tableName = Ember.Inflector.inflector.pluralize(camelized);
  ...
  function emberModel(type) {
    // TMF: Patch for AppKit requirement    
    var camelized = Ember.String.camelize(type.typeKey);      
    return App.get(camelized.capitalize());
  }
  ...
  var adapter = DS.Adapter.extend({
    ...
    findQuery: function(store, type, query, recordArray) {
      return getTable(type).then(function(dbTable) {
        var model = emberModel(type); // TMF: App.get(type.typeKey.capitalize());
